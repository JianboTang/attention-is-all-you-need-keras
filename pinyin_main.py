...
itokens, otokens = dd.MakeS2SDict('pinyin.corpus.txt', dict_file='pinyin_word.txt')
...
model_saver = ModelCheckpoint('pinyin.model.h5', monitor='ppl', save_best_only=True, save_weights_only=True)

#s2s.model.summary()
opt = Adam(0.001, 0.9, 0.98, epsilon=1e-9)
s2s.compile(opt)

try: s2s.model.load_weights('pinyin.model.h5')
except: print('\n\nnew model')

cmds = sys.argv[1:]
if 'train' in cmds:
	gen = dd.S2SDataGenerator('pinyin.corpus.txt', itokens, otokens, batch_size=32, max_len=120)
